...
def mae(prediction, target):
    return FloatTensor(
        [torch.sum(torch.abs(sanitize(prediction, target, return_diff=True)))]
    )
...
def plot(epoch_list, loss_prop_list, path):
    ...
    plt.plot(
        "x", "y1", data=df, color="green", linewidth=line_width, label="Train Loss"
    )
...
def args_parse():
    ...
    parser.add_argument(
        "--data-path", type=str, default="../data_1000/", help="Data Path"
    )
    ...
    parser.add_argument(
        "--n-conv", type=int, default=3, help="Number of Convolution Layers"
    )
    parser.add_argument(
        "--n-h",
        default=1,
    ...
    parser.add_argument(
        ...
        help="Path of the Pretrained CrysXPP Model Path",
    )
...
def main(seed):
    ...
    if len(full_dataset) < 2000:
        ...
        sample_data_list = [
            full_dataset[i] for i in tqdm(random.sample(range(len(full_dataset)), 2000))
        ]
    ...
    if os.path.isfile(args.property_model):
        ...
        checkpoint = torch.load(
            args.property_model, map_location=lambda storage, loc: storage
        )
        model.load_state_dict(checkpoint["state_dict"])
        normalizer.load_state_dict(checkpoint["normalizer"])
        print(
            "=> loaded model '{}' (epoch {}, validation {})".format(
                args.property_model, checkpoint["epoch"], checkpoint["best_mae_error"]
            )
        )
    else:
    ...
    for i in range(len(batch_data_test)):
        ...
        input_var = (
            Variable(atom_fea.to(device)),
            Variable(nbr_fea.to(device)),
